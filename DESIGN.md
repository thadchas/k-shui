...
The visual workspace for open-source, agent-driven Kafka management. The k-shui
Agent is the primary workflow, backed by the k-shui engine; direct management
pages remain visible expert controls and evidence destinations.
...
semantic colors. Dense-but-airy data UI: investigations, evidence cards, exact
operation previews, tables, stat tiles, charts, and drawers.
...
- **Topbar** 56px: breadcrumbs, Ask K-Shui, global search / command palette (⌘K), alerts bell with count, refresh interval selector (off/5s/30s/1m), time-range picker where relevant.
...
- The Agent panel opens from **Ask K-Shui** or a scoped **Investigate** action
  and expands to a dedicated investigation page without losing its cluster,
  resource, time window, provider/model, mode, or acting-user context.
- Keep evidence inspectable: show its resource link, retrieval time, freshness,
  limitations, and unavailable states. Visually distinguish observations,
  hypotheses, missing information, and next steps.
- Inspect mode has no execution control. Operate mode shows the exact target,
  parameters, effect, expiry, and verification state before enabling execution.
  Consequential actions use the required typed confirmation.
- Never present a model suggestion as completed work. Prepared, awaiting
  confirmation, running, verifying, succeeded, partially completed, failed,
  cancelled, expired, and outcome unknown states need distinct labels and
  recovery guidance.
- Keep deployment-disabled, authentication-required, read-only, permission,
  provider, and budget states explicit. Do not add client-side provider keys,
  arbitrary tool inputs, or claims beyond the implemented Agent MVP.
