...
async def _summaries(ctx: ClusterContext, group_types: list[str] | None = None) -> list[dict[str, Any]]:
    ...
    for g in listing:
        ...
        lag = sum(max(marks.get((o["topic"], o["partition"]), (0, 0))[1] - o["offset"], 0) for o in offsets)
        out.append(
            {
                "groupId": g["groupId"],
...
async def list_share_groups(
    ...
    try:
        items = await _summaries(ctx, group_types=["share"])
...
async def get_group(
    group_id: str, ctx: ClusterContext = Depends(get_cluster), principal: Principal = Depends(require_viewer)
) -> GroupDetail:
    admin = KafkaAdmin.get(ctx)
