...
3. `/c/:cluster/schemas/:subject`:
   - **Versions** — every version with its schema id and content.
   - **Diff** — unified diff between any two versions.
   - **Compatibility** — check a candidate schema against the subject's
     current mode before registering it for real.
   - Per-subject compatibility override, alongside the registry's global
     default.
