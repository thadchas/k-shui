...
1. `/c/:cluster/brokers` — table of brokers with status pill, rack, partition
   and leader counts, under-replicated count, and disk as a %-full bar
   (amber ≥ 75 %, red ≥ 90 %) when the Kafka client reports log-dir capacity,
   otherwise bytes used.
2. `/c/:cluster/brokers/:id` tabs:
   ...
   - **Log dirs** — per-log-dir size / capacity, offline dirs flagged
     (`error`), and a sortable, searchable per-partition breakdown
     (topic/partition/size/offset lag), largest first.
...
- Disk capacity (`totalBytes`/`usableBytes`) requires a `confluent-kafka`
  build with `describe_log_dirs`; on builds without it the fields are `null`
  and the UI shows bytes used only.
