...
Create `my-k-shui.yaml` in the repository root based on
`deploy/compose/k-shui.yaml`, add `auth` and
`agent` using [`../k-shui-agent.md`](../k-shui-agent.md), and mount it with a
local Compose override. Forward provider/auth secrets explicitly from a local
`.env` file or secret manager; `apiKeyEnv` in the YAML contains only the name of
the provider-key variable.
...
Start with `agent.allowMutations: false`, sign in as an admin, and test the
connection under **Settings → AI connections**. Do not scale the k-shui service while the agent
is enabled; current run admission and recovery require one application
worker/process. The Compose `connect` service being described as a “single
