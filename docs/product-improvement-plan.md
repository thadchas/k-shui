...
Recruit 5–8 representative operators/developers. Compare current and proposed flows for locating a failing component, finding a record, explaining lag, and understanding downstream impact. Measure task completion, task accuracy, time to first relevant evidence, setup completion (reaching a tested AI connection from a fresh install), time to first useful investigation (relevant evidence plus an understood next action), navigation reversals, and confidence in the next action. Establish a baseline first; a 30% reduction in median time to relevant evidence without lower completion accuracy remains a hypothesis, not a measured result.
...
Track user-rated usefulness, setup completion, time to first useful investigation, and repeat investigation use through moderated research sessions or opt-in content-free telemetry that excludes payloads, prompts, credentials, and sensitive resource identifiers. No new telemetry implementation is required to run the study. Collect qualitative reasons for unsuccessful investigations. Proposed targets are product hypotheses, not measured results.
